...
**`HITL-Task-Prototype-v9.html`** is the working version. Make edits here.
...
3. Skim `docs/HITL-Feedback.md` for the latest feedback shaping v9.

## Project shape

- Single-file HTML prototype — CSS and JS inline in `HITL-Task-Prototype-v9.html`.
...
## When asked to extend v9

Edit `HITL-Task-Prototype-v9.html` directly. Commit to a feature branch
(`steve/...` or `christina/...`), push, open a PR. Do not commit straight to `main`.

## When asked to start a new major version

1. Copy `HITL-Task-Prototype-v9.html` → `HITL-Task-Prototype-v10.html`.
2. `git mv HITL-Task-Prototype-v9.html archive/`.
3. Update `vercel.json` destination to `/HITL-Task-Prototype-v10.html`.
4. Update this file (`CLAUDE.md`) and `README.md` to reference v10 as active.
