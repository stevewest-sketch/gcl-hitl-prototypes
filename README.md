...
**`HITL-Task-Prototype-v9.html`** is the working version. All new edits land here.
...
open HITL-Task-Prototype-v9.html          # macOS — opens in default browser
```

Or, if you want live reload while editing, run any static server:

```bash
python3 -m http.server 8000               # then visit http://localhost:8000/HITL-Task-Prototype-v9.html
# or
npx serve .
```
...
```
.
├── HITL-Task-Prototype-v9.html   # Active working file — edit this
...
- Small fixes and polish → commit straight to the current version file (e.g. v9).
...
# ...edit HITL-Task-Prototype-v9.html...

git add HITL-Task-Prototype-v9.html
git commit -m "v9: tightened summary card copy"
...
git rebase main
# resolve any conflicts in v9.html, then:
...
- **Rough ownership zones.** If we're both going to touch v9 at the same time,
