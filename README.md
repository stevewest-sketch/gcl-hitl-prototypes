...
**`HITL-Task-Prototype-v14.html`** is the working version. All new edits land here.
Older versions (v1–v8, v12, v13) are in `archive/` for reference only.
...
open HITL-Task-Prototype-v14.html          # macOS — opens in default browser
```

Or, if you want live reload while editing, run any static server:

```bash
python3 -m http.server 8000               # then visit http://localhost:8000/HITL-Task-Prototype-v14.html
# or
npx serve .
```
...
```
.
├── HITL-Task-Prototype-v14.html   # Active working file — edit this
...
- Small fixes and polish → commit straight to the current version file (e.g. v14).
...
# ...edit HITL-Task-Prototype-v14.html...

git add HITL-Task-Prototype-v14.html
git commit -m "v14: tightened summary card copy"
...
git rebase main
# resolve any conflicts in v14.html, then:
...
- **Rough ownership zones.** If we're both going to touch v14 at the same time,
